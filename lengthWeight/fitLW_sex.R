...
stanOut_silver <- stan(file = "lengthWeight_no_gq.stan",
...
## run model with correlated structure
x_bighead  = dat3_bighead[ , model.matrix( ~ Sex * TLmL10 - 1)]
head(x_bighead)
...
stanOut_bighead <- stan(file = "lengthWeight_no_gq.stan",
                     data = stanData_bighead,
                     chains = 4, iter = n_iter,
                     control = list(adapt_delta = 0.8))
save(stanOut_bighead, dat3_bighead_pool_key, stanData_bighead,
     file = "lengthWeight_bighead_sex.RData")
